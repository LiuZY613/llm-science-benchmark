# code_debug / run1 — kimi (kimi-k2.6)

- elapsed: 119.9s
- exit_code: 0
- turns: 19
- stop_reason: end_turn
- usage: {'input_tokens': 12612, 'cache_creation_input_tokens': 0, 'cache_read_input_tokens': 300288, 'output_tokens': 5242, 'server_tool_use': {'web_search_requests': 0, 'web_fetch_requests': 0}, 'service_tier': 'standard', 'cache_creation': {'ephemeral_1h_input_tokens': 0, 'ephemeral_5m_input_tokens': 0}, 'inference_geo': '', 'iterations': [], 'speed': 'standard'}
- findings_md: yes
- verify_exit: 0
...
| # | 文件 | 问题 |
|---|------|------|
| 1 | `main.py:19-20` | `psi` 先创为实数数组再赋值复数，触发 `ComplexWarning` 并丢失虚部（动量相位） |
| 2 | `potentials.py:37` | 驱动势误用常量 `T_REF`（=0），未使用时间参数 `t`，导致驱动被冻结 |
| 3 | `propagator.py:21` | Strang 分裂算符的动能漂移步多除了 2，传播相位只有正确值的一半 |
| 4 | `grid.py:20` | 角波数直接用 `np.fft.fftfreq`（频率 f），缺少 `2π` 因子 |
| 5 | `observables.py:18` | 动量期望值公式中 `-i` 误写为 `+i`，符号取反 |

详细记录已写入 `buggy_project/findings.md`。
